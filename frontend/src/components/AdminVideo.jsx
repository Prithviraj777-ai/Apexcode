...
import { ArrowLeft, Video, Trash2, UploadCloud } from 'lucide-react';
...
const AdminVideo = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete the video solution linked to this problem?')) return;
    
    try {
      await axiosClient.delete(`/video/delete/${id}`);
      alert('Video solution successfully deleted.');
      fetchProblems(); // Reload list to update state if necessary
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to delete video solution.');
      console.log(err);
    }
  };
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl space-y-6 animate-in fade-in duration-200">
        ...
        <div className="border-b border-slate-850 pb-4">
          <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
            <Video size={22} className="text-indigo-450" /> Video Solution Manager
          </h1>
          <p className="text-xs text-slate-400 mt-1">Manage video editorial solutions and thumbnails for student tutorials.</p>
        </div>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex gap-2 justify-end">
                            <NavLink to={`/admin/upload/${problem._id}`}>
                              <Button 
                                size="sm" 
                                className="h-7 px-3 text-[10px] font-bold bg-indigo-600 hover:bg-indigo-500"
                              >
                                <UploadCloud size={11} className="mr-1" /> Upload
                              </Button>
                            </NavLink>
                            <Button 
                              variant="destructive" 
